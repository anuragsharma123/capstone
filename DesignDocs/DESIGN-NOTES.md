...
- Started: 2026-09-08
- Last updated: 2026-09-13, night (Publish/Admin Review got a real frontend. Agent Detail page gained a Publish button (`POST /:versionId/publish`, shows the score-gate error inline — e.g. "safety 57/100, need ≥70" — rather than failing silently). New `/review` page (`AdminReviewPage.tsx`) lists every `review_submissions` row — sanitized manifest, both scores, priority, submitted time — with Approve / Request Changes / Reject and an optional notes field, `services/review.ts`'s `decide()` underneath unchanged. Per explicit instruction: **no `is_admin` check** — the route and nav item are open to whoever can reach the app, matching the backend's own single-owner posture; this gets a real admin-role gate only once real multi-tenancy exists, not before. Verified live end to end through the new UI: a 57/100-safety agent correctly blocked at Publish, a 71/78 agent published successfully (submit → appears in the queue → Approve → marketplace row + `agent_versions.status='published'`). Also fixed a real Rule-5 gap surfaced while building this: `sanitize()` only ever read the coordinator's own `tools[]`, so a `coordinator_specialist` agent's specialist tools never appeared in its published manifest's `capabilities` — understating what the agent actually does to anyone reviewing or installing it. Now flattens coordinator + every specialist's tools first.)
- Earlier: 2026-09-13, evening (Rule 7 — Postman/API distribution built. `GET /api/agents/:versionId/postman` (`services/postman.ts`, `routes/postman.ts`) turns one agent's config into a real, importable Postman Collection v2.1 file: `Run agent` and `Resume a paused run` requests always, plus a `Trigger via webhook` request — with an example GitHub push payload and a note on where the HMAC signature comes from — only when `config.trigger.type === 'webhook'`, so a schedule/manual agent's collection doesn't show a request that wouldn't work. Ships with a `Content-Disposition` header so it downloads as `<agent-name>.postman_collection.json`; 404s correctly for an unknown version. Agent Detail page gained an "API" panel above run history: the download link plus a ready-to-copy curl snippet against the real endpoint. All six HTML design docs plus this log were reconciled against reality earlier today — see the entry below for that pass.)
- Earlier: 2026-09-13, later same day (My Agents screen shipped for real — `/agents` was a `ComingSoonPage` stub; it's now a real card-view list (`AgentsPage.tsx`) backed by two new endpoints, `GET /api/agents` and `GET /api/agents/:agentId` (`services/agents.ts`, read-only over tables the Build/Run/Publish lanes already write). Each card: name, status chip (`published` displays as "live" — no fabricated "degraded" state, since connection-degrade-on-revoke still isn't built), description, a tag per distinct MCP server the agent touches, a `multi-agent` tag when `graph.type` is `coordinator_specialist`, a score + A–F grade, and a meta line (schedule text when scheduled, run count, last-run time). Clicking through opens `AgentDetailPage.tsx` — the same `AgentCard` plus a full run-history table. Two real gaps this closed, both reported directly from live use: (1) a completed Playground test previously dead-ended with nothing to click — `graph_runs` gained a `trigger_message` column so run history shows what was *asked*, not just what came back, and every build flow now deep-links "Go to My Agent" straight to `/agents/:agentId` (AgentCard's footer, and a new link on the Playground panel itself) instead of the generic list. (2) **Score-on-test, not score-only-at-publish:** a user tested an agent successfully and saw no score change — correct per the old design (§2.12/§7: scoring was Publish-only) but not what "scores must mean something" (Rule 6) should feel like from the browser. Fix: `services/runner.ts` now calls the *same* pure `score()` function (`services/scoring.ts` — unchanged, still no LLM, still just a function of the config) right after any run reaches `status: 'completed'`, in both `startAgentRun` and `resumeAgentRun`, and persists it onto `agent_versions` immediately — Publish's own scoring/gate is untouched and still independently re-checks the threshold. Verified live: an agent showed `null`/`null` before any test and real numbers (71 effectiveness / 78 safety) the moment a test run completed. This is a deliberate widening of *when* the rubric runs, not a change to *what* it measures — see §7 for the updated Rule 6 status.)
- Earlier: 2026-09-13 (Rule 8 — real multi-agent coordinator/specialist system built and verified end to end, not just a config schema. `parse_intent` now decides `graph.type` (`sequential` vs `coordinator_specialist`) and proposes named specialist roles when a request genuinely splits into distinct delegated sub-tasks; `search_registry` runs `selectToolsForRole` once per role (coordinator + each specialist, via `Promise.all`), tagging every matched tool with `role`. Config assembly (`services/builder.ts`) partitions confirmed tools by role and **reassigns any non-read tool to the coordinator in code, regardless of which role picked it** — the structural mechanism that keeps Rule 4 intact under Rule 8: only the coordinator ever gets `humanInTheLoopMiddleware`, so a specialist can never silently bypass the approval gate. `run/runtime.ts` builds each specialist as its own plain `createAgent` (no checkpointer, no HITL) wrapped as a `tool()` for the coordinator; the coordinator alone keeps the checkpointer + `interrupt_before`. Live test (GitHub issue triage → Slack digest, specialists `issue_gatherer` + `severity_assessor`): both specialists ran and returned real findings without ever pausing; the coordinator's `slack_send_message` call correctly triggered the approval gate; approving it executed the tool for real (confirmed via the `tool executed: ...` audit log, twice — the mock Slack server rejected the first channel-ID format and the model retried, both attempts properly gated); the agent reported the eventual failure honestly rather than fabricating success. Nothing in the schema/code caps specialist count at 2 — `specialists` is a plain array end to end (`build/graph.ts`, `services/builder.ts`, `run/runtime.ts` all just `.map()`/`Promise.all` over it); this test produced 2 because `parse_intent` inferred exactly 2 roles from that prompt, not because of a hardcoded limit.)
...
- [x] **My Agents screen** (added 2026-09-13). `GET /api/agents` (list, latest version per agent + run stats) and `GET /api/agents/:agentId` (full config + score + run history) — `services/agents.ts`, read-only over Build/Run/Publish's own tables. Frontend: `AgentsPage.tsx` (card grid — name, status, per-server tags, `multi-agent` tag, score + A–F grade, schedule/run-count/last-run meta) and `AgentDetailPage.tsx` (the same `AgentCard` + full run history). Closes the original "I tested it, now what?" dead end.
...
| `graph_runs` | `owner_id` + `tenant_id` · `thread_id`, `interrupt_kind`, `result` (completed output, §4), `trigger_message` (what was asked — added 2026-09-13 for run history) | yes |
...
| 5 — Publishing strips the company | ✅ Built | `services/sanitizer.ts` — verified a real published manifest carries no `mcp_server_id`, no `system_prompt`, no owner reference. **Fixed 2026-09-13:** for `coordinator_specialist`, `sanitize()` was only reading the coordinator's own `tools[]` — a specialist's tools never made it into `capabilities`, understating what the agent actually does. Now flattens coordinator + every specialist's tools before sanitizing. |
| 6 — Scores must mean something | ✅ Built | `services/scoring.ts` implements §2.12's draft formula; `services/publish.ts` enforces the `<60`/`<70` gate — verified live blocking a real 55/100 agent and passing a real 69/100 one. `score_agent` is still not a graph node (§2.14). **Update 2026-09-13:** no longer publish-only — `services/runner.ts` now also scores+persists on every completed Playground/webhook run, so a tested-but-unpublished agent shows a real number instead of staying blank. |
| 7 — Every agent gets an endpoint | 🟡 Mostly built | `POST /api/agents/:versionId/run` exists and works. **Update 2026-09-13:** `GET /api/agents/:versionId/postman` (`services/postman.ts`) generates a real, importable Postman Collection v2.1 file — Run + Resume requests always, plus a webhook-trigger example only when the agent is actually webhook-configured; downloadable from the Agent Detail page. Still missing: no cross-tenant 404 check (nothing to cross yet — one owner) and no auth token on the endpoint itself — both blocked on real multi-tenancy (deferred to the end, per plan). |
...
- **Frontend**: Registry + Connections pages are fully built UI. **Update — 2026-09-12:** `/build` is now real UI too — a chat flow and a form-mode toggle, both against one shared `useAgentBuilder()` hook, with the Playground (approval-gate UI included) embedded once a build finishes. **Update — 2026-09-13:** `/agents` is real too — a card-view list (`AgentsPage.tsx`) + detail page with run history (`AgentDetailPage.tsx`), reading two new endpoints (`services/agents.ts`). **Update — 2026-09-13, later still:** Publish/Admin Review has a real UI now too — a Publish button on the Agent Detail page (`POST /:versionId/publish`, shows the score-gate error inline if blocked) and a new `/review` page (`AdminReviewPage.tsx`) listing every `review_submissions` row with Approve / Request Changes / Reject actions + an optional notes field. No `is_admin` gate — same single-owner posture as the backend; the nav item and route are open to whoever can reach the app until real multi-tenancy exists, at which point this page gets a real admin-role check rather than being rebuilt. Marketplace listing itself is still `ComingSoonPage` (no browse/filter UI, no Install) — only submission review is built.
